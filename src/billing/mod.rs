...
async fn check_rate_limit_redis(
    ...
    let now = Utc::now();
    let month_key = format!("ratelimit:{}:{}:{}", user.id, api_key.id, now.format("%Y-%m"));

    // Get current count from Redis
    let count: i64 = conn.get(&month_key).await.unwrap_or(0);

    info!("Redis rate limit check: user {} api_key {} count {}", user.id, api_key.id, count);
...
async fn check_rate_limit_db(
    ...
    info!("Checking rate limit (DB fallback) for user {} api_key {} from {} to {}",
...
async fn increment_redis_counter(user_id: i64, api_key_id: i64) -> Result<()> {
    ...
    // Get current month for key
    let now = Utc::now();
    let month_key = format!("ratelimit:{}:{}:{}", user_id, api_key_id, now.format("%Y-%m"));
